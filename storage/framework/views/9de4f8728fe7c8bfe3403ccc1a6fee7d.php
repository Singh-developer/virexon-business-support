    ...
    <header class="bg-white text-black fixed top-0 left-0 right-0 z-50 border-b border-slate-200 shadow-md w-full" style="height: 60px;">
        <div class="w-full px-4 lg:px-6 h-full flex items-center justify-between">

            <!-- Left: Mobile Menu Toggle + Logo & Brand -->
            <div class="flex items-center space-x-3">
                <!-- Mobile Hamburger Toggle Button -->
                <button id="mobile-menu-open-btn" class="lg:hidden text-slate-700 hover:text-black p-1.5 rounded-lg hover:bg-slate-100 transition focus:outline-none" aria-label="Open Mobile Menu">
                ...
                <a href="<?php echo e(route('dashboard')); ?>" class="flex items-center space-x-2">
                    <img src="<?php echo e(asset('images/virexon-logo.png')); ?>" alt="Logo" class="h-10 sm:h-12 object-contain">
                </a>
            </div>
            ...
            <div class="flex items-center space-x-3 sm:space-x-4 lg:space-x-6 text-xs lg:text-sm">
                <!-- Phone -->
                <div class="hidden md:flex items-center space-x-2 text-slate-700">
                    <svg class="w-4 h-4 text-[#2070E0]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                    </svg>
                    <!-- <span class="font-semibold text-black">0120-1234567</span> -->
                </div>
                ...
                <div class="hidden md:flex items-center space-x-2 text-slate-700">
                    ...
                    <span class="font-semibold text-black">support@virexon.in</span>
                </div>

                <!-- Bell Icon with Notifications -->
                <div class="relative" id="agent-notif-btn">
                    <button class="relative p-2 rounded-lg hover:bg-slate-100 transition focus:outline-none">
                        <svg class="w-5 h-5 text-slate-700 hover:text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <span class="absolute top-0.5 right-0.5 bg-red-500 text-white text-[9px] font-bold w-4 h-4 rounded-full flex items-center justify-center border-2 border-white">
                    ...
                    <div id="agent-notif-dropdown" class="fixed sm:absolute sm:right-0 mt-14 sm:mt-2 sm:top-full left-4 right-4 sm:left-auto sm:w-80 bg-white rounded-xl shadow-2xl hidden border border-slate-200 z-[60] overflow-hidden" style="cursor: default;">
                        <div class="flex justify-between items-center px-4 py-3 border-b border-slate-100 bg-slate-50">
                            ...
                            <?php if(Auth::check() && Auth::user()->unreadNotifications->count() > 0): ?>
                            <form method="POST" action="<?php echo e(route('notifications.mark-read')); ?>" class="m-0" onclick="event.stopPropagation()">
                                <?php echo csrf_field(); ?>
                ...
                <div class="hidden sm:flex items-center space-x-1.5 bg-slate-100 hover:bg-slate-200 px-3 py-1.5 rounded text-slate-700 text-xs cursor-pointer border border-slate-200">
                    <span>Agent ID:</span>
                    <span class="font-bold text-black"><?php echo e(Auth::check() && Auth::user()->detail ? Auth::user()->detail->agent_id_number : 'N/A'); ?></span>
                </div>

                <!-- User Profile Dropdown -->
                <div class="relative flex items-center space-x-2 cursor-pointer sm:pl-1" id="header-profile-btn">
                    <div class="w-8 h-8 rounded-full bg-[#0D3C8A] flex items-center justify-center text-white font-bold overflow-hidden shrink-0 shadow">
                        <svg class="w-6 h-6 text-white mt-1" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd" />
                        </svg>
                    </div>
                    <div class="hidden xl:block text-left leading-tight">
                        <div class="font-bold text-xs text-black"><?php echo e(Auth::check() ? Auth::user()->name : 'Guest'); ?></div>
                        <div class="text-[11px] text-slate-500"><?php echo e(Auth::check() && Auth::user()->role ? Auth::user()->role->name : 'Working Agent'); ?></div>
                    </div>
            ...
            <div>
                ...
                <div class="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
                    <div class="flex items-center space-x-2">
                        ...
                        <div><a href="<?php echo e(route('dashboard')); ?>"><img src="<?php echo e(asset('images/virexon-logo.png')); ?>" alt="Virexon" style="max-width: 40%;" /></a></div>
                ...
                <nav class="space-y-1.5 text-xs font-semibold">
                    ...
                    <a href="<?php echo e(route('payments.index')); ?>" class="flex items-center space-x-3 px-4 py-3 rounded-lg text-slate-600 hover:bg-slate-50 transition">
                        <svg class="w-4 h-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <?php $agentSanctionAlerts = auth()->user()->sanctionLetters()->where('review_status', 'reupload_required')->count(); ?>
                    <a href="<?php echo e(route('agent.sanctions.index')); ?>" class="flex items-center justify-between px-4 py-3 rounded-lg text-slate-600 hover:bg-slate-50 transition">
                        <div class="flex items-center space-x-3">
                            <svg class="w-4 h-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <?php if($agentSanctionAlerts > 0): ?>
                        <span class="bg-orange-500 text-white text-[10px] font-bold w-5 h-5 rounded-full flex items-center justify-center"><?php echo e($agentSanctionAlerts); ?></span>
                        <?php endif; ?>
                    </a>
                    ...
                    <button type="button" id="mobile-notif-btn" class="flex items-center justify-between w-full px-4 py-3 rounded-lg text-slate-600 hover:bg-slate-50 transition text-left">
                        <div class="flex items-center space-x-3">
                            <svg class="w-4 h-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 01-6 0v-1m6 0H9" />
                            </svg>
                        ...
                        <?php if(Auth::check() && Auth::user()->unreadNotifications->count() > 0): ?>
                        <span class="bg-red-500 text-white text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center"><?php echo e(Auth::user()->unreadNotifications->count() > 9 ? '9+' : Auth::user()->unreadNotifications->count()); ?></span>
                        <?php endif; ?>
                    </button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const sidebarNotifBtn = document.getElementById('sidebar-notif-btn');

            function toggleNotifDropdown(e) {
                if (e) e.stopPropagation();
                if (agentNotifDropdown) agentNotifDropdown.classList.toggle('hidden');
            }

            if (agentNotifBtn) agentNotifBtn.addEventListener('click', toggleNotifDropdown);
            if (sidebarNotifBtn) sidebarNotifBtn.addEventListener('click', toggleNotifDropdown);

            // Mobile Notification Button - close drawer then show dropdown
            ...
            if (mobileNotifBtn) {
                mobileNotifBtn.addEventListener('click', function(e) {
                    ...
                    closeDrawer();
                    setTimeout(function() {
                        if (agentNotifDropdown) agentNotifDropdown.classList.remove('hidden');
                    }, 100);
                });
            }
            ...
            document.addEventListener('click', function(e) {
                ...
                if (agentNotifDropdown && !agentNotifDropdown.classList.contains('hidden') && !agentNotifBtn.contains(e.target) && !(sidebarNotifBtn && sidebarNotifBtn.contains(e.target))) {
