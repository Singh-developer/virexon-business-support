...
<div class="page-head" style="margin-bottom:20px; flex-wrap:wrap; gap:12px;">
    <div>
        <a href="<?php echo e(route('sanctions.index')); ?>" style="color:#64748b;text-decoration:none;font-size:13px;display:inline-flex;align-items:center;gap:4px;margin-bottom:8px;">
            ← Back to Sanction Letters
        </a>
        <div class="eyebrow">SIGNED PDF REVIEW</div>
        <h1 style="font-size:22px;margin-bottom:4px;">
            <?php echo e($letter->sanction_letter_no); ?>

        </h1>
        <p style="color:#64748b;font-size:13px;margin:0;">
            <?php echo e($letter->user->name); ?>

            (Agent ID: <strong><?php echo e($letter->user->detail?->agent_id_number ?? 'N/A'); ?></strong>)
            &nbsp;|&nbsp; <?php echo e($letter->user->email); ?>

        </p>
    </div>
    <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;">
        <?php $ws = $letter->workflowStatus(); ?>
        <span class="status-pill <?php echo e($ws['key']); ?>"><?php echo e($ws['label']); ?></span>
        <a href="<?php echo e(route('sanctions.show', $letter)); ?>" class="btn secondary tiny">Letter Details</a>
        <a href="<?php echo e(route('sanctions.download', $letter)); ?>" class="btn secondary tiny">Original PDF</a>
    </div>
</div>
...
<div class="flash error" style="margin-bottom:16px;">✗ <?php echo e(session('error')); ?></div>
<?php endif; ?>
...
<div class="rv-main-grid">
    ...
    <div>
        <div class="rv-panel" style="padding:0 20px;">
            ...
            <div class="rv-fact">
                ...
                <?php if($letter->status === 'sent' && $letter->sent_at): ?>
                    <span class="v"><span class="ws-yes">Yes</span><br><small style="color:#94a3b8;font-weight:400;"><?php echo e($letter->sent_at->format('d M Y, h:i A')); ?></small></span>
                <?php else: ?>
            ...
            <div class="rv-fact">
                ...
                <?php if($letter->downloaded_at): ?>
                    <span class="v"><span class="ws-yes">Yes</span><br><small style="color:#94a3b8;font-weight:400;"><?php echo e($letter->downloaded_at->format('d M Y, h:i A')); ?></small></span>
                <?php else: ?>
                    <span class="v"><span class="ws-no">No</span></span>
                <?php endif; ?>
            </div>
            <div class="rv-fact">
                ...
                <?php if(($letter->signed_pdf_upload_count ?? 0) > 0): ?>
                    <span class="v"><span class="ws-yes">Yes</span><br><small style="color:#94a3b8;font-weight:400;"><?php echo e($letter->signed_pdf_uploaded_at?->format('d M Y, h:i A')); ?></small></span>
                <?php else: ?>
                    <span class="v"><span class="ws-no">No</span></span>
                <?php endif; ?>
            </div>
            <div class="rv-fact">
                ...
                <span class="v"><span class="status-pill <?php echo e($ws['key']); ?>"><?php echo e($ws['label']); ?></span></span>
            </div>
            <?php if($letter->reviewed_by || $letter->reviewed_at): ?>
            <div class="rv-fact">
                <span class="k">Reviewed By / At</span>
                <span class="v"><small style="font-weight:400;color:#94a3b8;"><?php echo e($letter->reviewedBy?->name ?? '—'); ?><br><?php echo e($letter->reviewed_at?->format('d M Y, h:i A')); ?></small></span>
            </div>
            <?php endif; ?>
        </div>
        ...
        <div class="rv-panel">
            ...
            <?php elseif($letter->review_status === 'approved'): ?>
            ...
            <?php else: ?>
                <form method="POST" action="<?php echo e(route('sanctions.approve', $letter)); ?>" style="margin-bottom:14px;">
                    <?php echo csrf_field(); ?>
                    ...
                    <textarea name="review_comment" rows="2" class="note-input" placeholder="e.g. Signed copy verified & accepted."><?php echo e($letter->review_comment); ?></textarea>
                    <div style="margin-top:10px;">
                        <button type="submit" class="btn secondary tiny" style="width:100%;justify-content:center;">✓ Approve Signed Copy</button>
                    </div>
                </form>

                <form method="POST" action="<?php echo e(route('sanctions.reupload-required', $letter)); ?>" style="margin-bottom:14px;">
                    <?php echo csrf_field(); ?>
                    ...
                    <textarea name="review_comment" rows="2" class="note-input" placeholder="e.g. Signature is not clearly visible. Please sign again and re-upload."><?php echo e($letter->review_status === 'reupload_required' ? $letter->review_comment : ''); ?></textarea>
                    <div style="margin-top:10px;">
            ...
            <form method="POST" action="<?php echo e(route('sanctions.status', $letter)); ?>" style="border-top:1px dashed #e2e8f0;padding-top:12px;">
                <?php echo csrf_field(); ?>
                ...
                <select name="review_status" class="note-input" style="margin-bottom:8px;">
                    <option value="pending" <?php echo e($letter->review_status === 'pending' ? 'selected' : ''); ?>>Pending</option>
                    <option value="under_review" <?php echo e($letter->review_status === 'under_review' ? 'selected' : ''); ?>>Under Review</option>
                    <option value="reupload_required" <?php echo e($letter->review_status === 'reupload_required' ? 'selected' : ''); ?>>Re-upload Required (reopens 48h upload slot)</option>
                    <option value="approved" <?php echo e($letter->review_status === 'approved' ? 'selected' : ''); ?>>Approved</option>
                </select>
                <textarea name="review_comment" rows="2" class="note-input" placeholder="Note for agent (optional)"><?php echo e($letter->review_comment); ?></textarea>
                <div style="margin-top:10px;">
    ...
    <div>
        <?php if($letter->latestUpload): ?>
        <?php $latest = $letter->latestUpload; ?>
        <div id="signedPreviewPanel" class="rv-panel" style="padding:0 0 16px; scroll-margin-top:90px;">
            <div style="display:flex;justify-content:space-between;align-items:center;padding:14px 20px;border-bottom:1px solid #e2e8f0;margin-bottom:14px;flex-wrap:wrap;gap:8px;">
                ...
                <div style="display:flex;gap:8px;">
                    <a id="signedPreviewView" href="<?php echo e(route('sanctions.signed-pdf', [$letter, $latest])); ?>" target="_blank" class="btn secondary tiny">View</a>
                    <a id="signedPreviewDownload" href="<?php echo e(route('sanctions.signed-download', [$letter, $latest])); ?>" class="btn secondary tiny">Download</a>
                </div>
            </div>

            <?php if($letter->review_status === 'reupload_required'): ?>
            <div style="margin:0 20px 14px;background:#fffbeb;border:1px solid #fde68a;border-radius:8px;padding:10px 14px;font-size:12px;color:#92400e;">
                ⚠️ This upload is awaiting a <strong>re-upload</strong>. You asked <?php echo e($letter->user->name); ?> to upload a corrected signed PDF.
            </div>
            <?php endif; ?>
            ...
            <div style="margin:0 20px;border:1px solid #e2e8f0;border-radius:8px;overflow:hidden;background:#f8fafc;">
                ...
                ?>
                <a id="signedPreviewImgLink" href="<?php echo e($latestUrl); ?>" target="_blank" style="display:<?php echo e($latestIsImage ? 'block' : 'none'); ?>;">
                    <img id="signedPreviewImg" src="<?php echo e($latestUrl); ?>" alt="Signed copy" style="width:100%;max-height:640px;object-fit:contain;display:block;background:#fff;">
                </a>
                <iframe id="signedPreviewFrame" src="<?php echo e($latestUrl); ?>" style="width:100%;height:640px;border:0;<?php echo e($latestIsImage ? 'display:none;' : ''); ?>" title="Signed Sanction Letter"></iframe>
            </div>

            <div id="signedPreviewCaption" style="margin-top:12px;padding:0 20px;font-size:12px;color:#64748b;"
                 data-name="<?php echo e($latest->original_name ?? 'signed-sanction-letter.pdf'); ?>"
                 data-uploader="<?php echo e($latest->uploader?->name ?? ''); ?>">
                📎 <?php echo e($latest->original_name ?? 'signed-sanction-letter.pdf'); ?>

                &nbsp;·&nbsp; Uploaded <?php echo e($latest->uploaded_at?->format('d M Y, h:i A') ?? $latest->created_at->format('d M Y, h:i A')); ?>

                <?php if($latest->file_size): ?> &nbsp;·&nbsp; <?php echo e(number_format($latest->file_size / 1024, 1)); ?> KB <?php endif; ?>
                <?php if($latest->uploader): ?> &nbsp;·&nbsp; by <?php echo e($latest->uploader->name); ?> <?php endif; ?>
            </div>
        </div>
        <?php else: ?>
        <div class="rv-panel">
        ...
        <div class="rv-panel">
            ...
            <table style="width:100%;border-collapse:collapse;">
                ...
                <tbody>
                    <?php $__currentLoopData = $letter->uploads; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $idx => $upload): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <tr style="border-bottom:1px solid #f1f5f9;font-size:13px;">
                        <td style="padding:8px;">
                            <?php echo e($letter->uploads->count() - $idx); ?>

                            <?php if($upload->id === $letter->latestUpload?->id): ?>
                            <span class="status-pill pending" style="margin-left:4px;padding:1px 8px;font-size:10px;">LATEST</span>
                            <?php endif; ?>
                        </td>
                        <td style="padding:8px;white-space:nowrap;"><?php echo e($upload->uploaded_at?->format('d M Y, h:i A') ?? $upload->created_at->format('d M Y, h:i A')); ?></td>
                        <td style="padding:8px;max-width:220px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
                            <?php echo e($upload->original_name ?? $upload->file_path); ?>

                        </td>
                        <td style="padding:8px;white-space:nowrap;"><?php echo e($upload->uploader?->name ?? '—'); ?></td>
                        <td style="padding:8px;">
                            <a href="<?php echo e(route('sanctions.signed-pdf', [$letter, $upload])); ?>" class="btn secondary tiny signed-history-view"
                               data-url="<?php echo e(route('sanctions.signed-pdf', [$letter, $upload])); ?>"
                               data-download="<?php echo e(route('sanctions.signed-download', [$letter, $upload])); ?>"
                               data-name="<?php echo e($upload->original_name ?? 'signed file'); ?>"
                               data-date="<?php echo e($upload->uploaded_at?->format('d M Y, h:i A') ?? $upload->created_at->format('d M Y, h:i A')); ?>"
                               data-size="<?php echo e($upload->file_size ? number_format($upload->file_size / 1024, 1) . ' KB' : ''); ?>"
                               data-uploader="<?php echo e($upload->uploader?->name ?? ''); ?>"
                               data-ext="<?php echo e(strtolower(pathinfo($upload->file_path ?? $upload->original_name ?? '', PATHINFO_EXTENSION))); ?>"
                               data-is-latest="<?php echo e($upload->id === $letter->latestUpload?->id ? '1' : '0'); ?>">View</a>
                            <a href="<?php echo e(route('sanctions.signed-download', [$letter, $upload])); ?>" class="btn secondary tiny" style="margin-left:4px;">Download</a>
                        </td>
                    </tr>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\xampp\htdocs\laravel\agent-business-support\resources\views/sanctions/review.blade.php ENDPATH**/ ?>