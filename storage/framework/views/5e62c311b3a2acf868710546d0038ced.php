...
.logo-wrap { width: 132pt; text-align: center; }
.logo-wrap img { display: block; margin-left: 17pt; margin-right: auto; }
.tagline {
    margin-top: 4pt;
    font-size: 7pt;
    font-weight: bold;
...
.recipient { margin-top: 10pt; font-size: 9.8pt; line-height: 1.5; }

.salutation { margin-top: 17pt; font-size: 9.8pt; }

.intro { margin-top: 5pt; font-size: 9.8pt; line-height: 1.4; text-align: justify; }

.details-table { width: 100%; margin-top: 14pt; border-collapse: collapse; border: 1pt solid #9f8ab9; font-size: 9pt; }
.details-table th {
    background-color: #002060; color: #ffffff; font-weight: bold;
    text-align: left; padding: 1.5pt 8pt 1.5pt 14pt;
    border: 1pt solid #9f8ab9;
    border-right: 1pt solid #8db3e2;
    letter-spacing: 0.3pt;
    width: 240pt;
}
.details-table th.det { border-left: 1pt solid #8db3e2; border-right: 1pt solid #9f8ab9; }
.details-table td { padding: 1.5pt 8pt 1.5pt 6pt; border: 1pt solid #9f8ab9; border-right: 1pt solid #8db3e2; vertical-align: middle; }
.details-table td.det { border-left: 1pt solid #8db3e2; border-right: 1pt solid #9f8ab9; }
.details-table tr.alt td { background-color: #dfd8e8; }
.details-table td.amt { font-weight: bold; }

.req { margin-top: 9pt; font-size: 9pt; line-height: 1.4; text-align: justify; }

.credit-head { margin-top: 10pt; font-size: 9.8pt; font-weight: bold; }
...
.sig-row { width: 100%; margin-top: 14pt; font-size: 9.8pt; }
...
<?php
    ...
    $agentIdStr = $df['agent_id'] ?? ($agent->detail?->agent_id_number ?? '');
    $sanctionNo = (string) ($df['sanction_number'] ?? ($df['letter_no'] ?? ''));
    if (preg_match('/(\d+)$/', $sanctionNo, $m)) {
        $sanctionNo = $m[1];
    }
    $sanctionNo = str_pad($sanctionNo, 6, '0', STR_PAD_LEFT);
    $referenceYear = now()->format('Y');
    $referenceNo = 'RFE/AF/' . $referenceYear . '/' . $sanctionNo;
    $sanctionLetterNo = 'V-EOM/SL/AF/' . $referenceYear . '/' . $sanctionNo;

    $letterAddress = trim($df['letter_address'] ?? '');
    if ($letterAddress === '') {
        $letterAddress = $agent->detail?->current_address ?? '';
        if ($agent->detail?->current_city) $letterAddress .= ', ' . $agent->detail->current_city;
        if ($agent->detail?->current_state) $letterAddress .= ', ' . $agent->detail->current_state;
        if ($agent->detail?->current_pincode) $letterAddress .= ' - ' . $agent->detail->current_pincode;
    }

    $sanctionDate = $df['sanction_date'] ?? now()->format('d/m/Y');
    ...
    $approvedAmount = $df['approved_amount'] ?? 0;
    $disbursementMode = $df['disbursement_mode'] ?? 'Bank Account Transfer';
    $processFee = $df['process_fee'] ?? '';
    $tenure = $df['tenure'] ?? '';
    $monthlyPrincipal = $df['monthly_principal_settlement'] ?? '';
    $monthlyCharges = $df['monthly_portal_service_charges'] ?? '';
    $totalMonthly = $df['total_monthly_settlement'] ?? '';
...
<div class="p1">
    ...
    <table class="hdr">
        <tr>
            ...
            <td class="hdr-info">
                <div class="box">
                    <div><span class="lbl">Address::-</span><span class="val"> Plot No. 6-10-3/5, Plot No. 25,<br>Balanagar, Hyderabad -500043</span></div>
                    <div><span class="lbl">E-mail ID: -</span><span class="lnk"> support@finance.virexon.in</span></div>
                    <div><span class="lbl">Portal Website: -</span><span class="lnk"> https://finance.virexon.in</span></div>
                    <div><span class="lbl">Website: -</span><span class="lnk"> https://virexon.in</span></div>
                </div>
            </td>
        </tr>
    </table>
    ...
    <div class="body-wrap">
        ...
        <table class="ref-row">
            <tr>
                <td>Reference No:- <?php echo e($referenceNo); ?></td>
                <td class="right">Date :- <?php echo e($sanctionDate); ?></td>
            </tr>
        </table>

        <div class="recipient">
            <div>Mr. <?php echo e($agent->name); ?></div>
            <div><?php echo e((isset($guardian_relation) ? $guardian_relation : (isset($df['guardian_relation']) ? $df['guardian_relation'] : 'S/o')) . ':-'); ?> <?php if($guardian): ?> <?php echo e($guardian); ?> <?php endif; ?></div>
            <div>Agent ID:- <?php echo e($agentIdStr); ?></div>
            <div>Address:- <?php echo e($letterAddress); ?></div>
        </div>
        ...
        <table class="details-table">
            ...
            <tbody>
                <tr class="alt"><td><strong>Agent Name</strong></td><td class="det"><?php echo e($agent->name); ?></td></tr>
                <tr><td><strong>Agent ID</strong></td><td class="det"><?php echo e($agentIdStr); ?></td></tr>
                <tr class="alt"><td><strong>Sanction Letter No.</strong></td><td class="det"><?php echo e($sanctionLetterNo); ?></td></tr>
                <tr><td><strong>Approved A/F Amount</strong></td><td class="det amt"><?php echo e($fmt($approvedAmount)); ?></td></tr>
                <tr class="alt"><td><strong>Disbursement Mode</strong></td><td class="det"><?php echo e($disbursementMode); ?></td></tr>
                <tr><td><strong>Process Fee</strong></td><td class="det"><?php echo e($fmt($processFee)); ?></td></tr>
                <tr><td><strong>Tenure</strong></td><td class="det"><?php echo e($tenure); ?></td></tr>
                <tr class="alt"><td><strong>Monthly Principal Settlement</strong></td><td class="det amt"><?php echo e($fmt($monthlyPrincipal)); ?></td></tr>
                <tr><td><strong>Monthly Portal &amp; Service Charges</strong></td><td class="det amt"><?php echo e($fmt($monthlyCharges)); ?></td></tr>
                <tr class="alt"><td><strong>Total Monthly Settlement</strong></td><td class="det amt"><?php echo e($fmt($totalMonthly)); ?></td></tr>
            </tbody>
        </table>

        <div class="req">
            Your are requested to go through the terms &amp; conditions of sanction and one copy of the same be signed and returned to us as an acceptance of the terms &amp; conditions of sanction.
        </div>
        ...
        <div class="credit-list">
            ...
            <div class="cb">&bull;&nbsp;&nbsp;To properly complete, execute, and sign all required documents relating to the A/F Sanction.</div>
...
<div class="terms">
    ...
    <div class="term">&bull;&nbsp;&nbsp;The Agent shall be liable to pay the applicable Portal Charges prescribed by the Company for portal processing and related administrative/processing services. Such Portal Charges shall be strictly non-refundable, non-adjustable and non-recoverable, irrespective of whether the proposed A/F is subsequently approved declined, withheld, suspended, canceled or not released for any reason whatsoever, subject to applicable law.</div>
        ...
        <table style="width:100%;">
            <tr>
                ...
                <td style="text-align:right; vertical-align:bottom;">
                    <span class="rgt">(<?php echo e($agent->name); ?>)&nbsp;&nbsp;Agent ID (<?php echo e($agentIdStr); ?>)</span>
                </td>
            </tr>
        </table>
    </div>
...
<div class="pfoot2">Page 2-2</div>

</div>

</body>
</html><?php /**PATH E:\xampp\htdocs\laravel\agent-business-support\resources\views/sanctions/pdf.blade.php ENDPATH**/ ?>