...
<div class="page-head">
    ...
    <a class="btn secondary" href="{{ route('sanctions.index') }}">← All Letters</a>
</div>

<form id="sanctionForm" method="POST" action="{{ route('sanctions.store') }}" enctype="multipart/form-data">
    @csrf
    ...
    <div class="form-container">
        ...
        <div class="section-grid">
            ...
            <div class="card">
                ...
                <div class="row-2">
                    <div class="field">
                        ...
                        <input id="agent_name" name="agent_name" type="text" value="{{ old('agent_name', $prefillName) }}" readonly placeholder="Auto-filled">
                    </div>
                    <div class="field">
                        <label for="agent_id_display">Agent ID</label>
                        <input id="agent_id_display" type="text" value="{{ old('agent_id_display', $prefillAgentId) }}" readonly placeholder="Auto-filled">
                    </div>
                </div>
                ...
                <div class="row-2">
                    <div class="field">
                        ...
                        <select id="guardian_relation" name="guardian_relation">
                            <option value="S/o" {{ old('guardian_relation', 'S/o') === 'S/o' ? 'selected' : '' }}>S/o (Son of)</option>
                            <option value="D/o" {{ old('guardian_relation') === 'D/o' ? 'selected' : '' }}>D/o (Daughter of)</option>
                            <option value="W/o" {{ old('guardian_relation') === 'W/o' ? 'selected' : '' }}>W/o (Wife of)</option>
                        </select>
                    </div>
                    <div class="field">
                        <label for="sanction_date">Sanction Date</label>
                        <input id="sanction_date" name="sanction_date" type="date" required value="{{ old('sanction_date', date('Y-m-d')) }}">
                    </div>
                </div>

                <div class="field">
                    <label for="letter_address">Recipient Address</label>
                    <textarea id="letter_address" name="letter_address" required rows="3" placeholder="Auto-filled — editable for letter">{{ old('letter_address', $prefillAddress) }}</textarea>
            ...
            <div class="card">
                ...
                <div class="field">
                    ...
                    <input id="sanction_number_display" type="text" value="{{ $nextSanctionNumber }}" readonly>
                    <div class="hint">Reference No: <code>{{ $nextReferenceNumber }}</code></div>
                </div>
                <div class="row-2">
                    <div class="field">
                    ...
                    <div class="field">
                        ...
                        <select id="disbursement_mode" name="disbursement_mode" required>
                            ...
                            @foreach($disbursementModes as $mode)
                            <option value="{{ $mode }}" {{ old('disbursement_mode') === $mode ? 'selected' : '' }}>{{ $mode }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="row-2">
                    <div class="field">
                        ...
                        <input id="tenure" name="tenure" type="number" min="1" step="1" required value="{{ old('tenure') }}" placeholder="e.g. 12">
                    </div>
                    <div class="field">
                        <label for="process_fee">Process Fee (₹)</label>
                        <input id="process_fee" name="process_fee" type="number" step="0.01" min="0" value="{{ old('process_fee') }}" placeholder="0.00">
                    </div>
                </div>

                <div class="field">
                    <label for="monthly_principal_settlement">Monthly Principal Settlement (₹)</label>
                    <input id="monthly_principal_settlement" name="monthly_principal_settlement" type="number" step="0.01" min="0" required value="{{ old('monthly_principal_settlement') }}" placeholder="0.00">
                </div>
                <div class="field">
                    <label for="monthly_portal_service_charges">Monthly Portal &amp; Service Charges (₹)</label>
                    <input id="monthly_portal_service_charges" name="monthly_portal_service_charges" type="number" step="0.01" min="0" required value="{{ old('monthly_portal_service_charges') }}" placeholder="0.00">
                </div>
                <div class="field">
            ...
            <div class="card">
                ...
                <div class="row-2">
                    ...
                    <div class="field">
                        ...
                        <input id="signature_company" name="signature_company" type="text" maxlength="180" value="{{ old('signature_company', 'Virexon') }}">
                    </div>
                </div>
                <div class="field">
        ...
        <div class="action-bar">
            <label style="display:flex; align-items:center; gap:8px; cursor:pointer; font-weight:600; color:var(--text-main); margin-right:auto;">
                <input type="checkbox" name="send_email" id="send_email" value="1" {{ old('send_email') ? 'checked' : '' }} style="width:18px; height:18px; accent-color:var(--primary);">
...
downloadBtn.addEventListener('click', function(){
    ...
    form.action = "{{ route('sanctions.download-pdf') }}";
    form.target = '_blank';
    form.submit();
